...
var createAdaptorServer = (options) => {
  ...
  const createServer = options.createServer || require$$0.createServer;
  ...
  insert(tokens, index, paramMap, context, pathErrorCheckOnly) {
    ...
    if (pattern) {
      ...
      if (!node2) {
        ...
        node2 = this.#children[regexpStr] = new Node$1();
        if (name2 !== "") {
          node2.#varIndex = context.varIndex++;
        }
      }
      if (!pathErrorCheckOnly && name2 !== "") {
        paramMap.push([name2, node2.#varIndex]);
      }
    } else {
      ...
      if (!node2) {
        ...
        node2 = this.#children[token] = new Node$1();
      }
    }
...
var getStats = (path) => {
  let stats;
  try {
    stats = require$$0$1.lstatSync(path);
  } catch {
  }
  return stats;
};
var serveStatic = (options = { root: "" }) => {
  ...
  return async (c, next) => {
    ...
    if (stats && stats.isDirectory()) {
      ...
      path = require$$2.join(path, indexFile);
      stats = getStats(path);
    }
    if (!stats) {
    ...
    if (!range) {
      c.header("Content-Length", size.toString());
      return c.body(createStreamBody(require$$0$1.createReadStream(path)), 200);
    }
    ...
    const stream2 = require$$0$1.createReadStream(path, { start, end });
...
function requireLogger() {
  ...
  class Logger {
    ...
    constructor({
      ...
      errorHandler2?.setOptions({ ...dependencies, logFn: this.error });
...
function requireFile$1() {
  ...
  const fs2 = require$$0$1;
  const os = require$$1;
  class File2 extends EventEmitter {
    asyncWriteQueue = [];
    bytesWritten = 0;
...
function registerAppListeners(mainWindow2) {
  require$$0$5.ipcMain.handle("app:version", () => {
    return require$$0$5.app.getVersion();
  });
  require$$0$5.ipcMain.handle("app:platform", () => {
    return process.platform;
  });
}
function registerWindowListeners(mainWindow2) {
  require$$0$5.ipcMain.on("window:minimize", () => {
    mainWindow2.minimize();
  });
  require$$0$5.ipcMain.on("window:maximize", () => {
  ...
  require$$0$5.ipcMain.on("window:close", () => {
    mainWindow2.close();
  });
  require$$0$5.ipcMain.handle("window:is-maximized", () => {
    return mainWindow2.isMaximized();
  });
}
function registerFileListeners(mainWindow2) {
  require$$0$5.ipcMain.handle("dialog:open", async (event, options = {}) => {
    const result = await require$$0$5.dialog.showOpenDialog(mainWindow2, {
      properties: ["openFile"],
      filters: [
  ...
  require$$0$5.ipcMain.handle("dialog:save", async (event, data) => {
    const result = await require$$0$5.dialog.showSaveDialog(mainWindow2, {
      filters: [
        { name: "JSON Files", extensions: ["json"] },
        { name: "All Files", extensions: ["*"] }
      ]
  ...
  require$$0$5.ipcMain.handle("dialog:message", async (event, options) => {
    return await require$$0$5.dialog.showMessageBox(mainWindow2, options);
  });
}
...
log.info(`App packaged: ${require$$0$5.app.isPackaged}`);
distExports.updateElectronApp();
if (process.platform === "win32") {
  require$$0$5.app.setAppUserModelId("com.mcpjam.inspector");
}
let mainWindow = null;
let server = null;
let serverPort = 0;
...
function createSimpleHonoApp() {
  ...
  if (!app2.isPackaged) {
    ...
    app2.use("/*", serveStatic({ root: clientPath }));
    app2.get("*", (c) => {
...
function createMainWindow(serverUrl) {
  const window2 = new require$$0$5.BrowserWindow({
    width: 1400,
    height: 900,
    minWidth: 800,
    minHeight: 600,
    icon: require$$2.join(__dirname, "../assets/icon.png"),
    // You can add an icon later
    webPreferences: {
      nodeIntegration: false,
      contextIsolation: true,
      enableRemoteModule: false,
      preload: require$$2.join(__dirname, "../build/preload.js")
    },
    titleBarStyle: process.platform === "darwin" ? "hiddenInset" : "default",
    show: false
    // Don't show until ready
  });
  registerListeners(window2);
  ...
  if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
    window2.loadURL(MAIN_WINDOW_VITE_DEV_SERVER_URL);
  } else {
    const rendererPath = require$$2.join(__dirname, `../renderer/${MAIN_WINDOW_VITE_NAME}/index.html`);
    try {
      const fs2 = require("fs");
      if (fs2.existsSync(rendererPath)) {
        window2.loadFile(rendererPath);
      } else {
        log.info("Renderer files not in expected location, using embedded server");
        window2.loadURL(serverUrl);
      }
    } catch (error) {
...
require$$0$5.app.on("activate", async () => {
...
require$$0$5.app.on("web-contents-created", (_, contents) => {
  contents.on("new-window", (navigationEvent, navigationUrl) => {
    ...
    require$$0$5.shell.openExternal(navigationUrl);
  });
});
...
if (!gotTheLock) {
  ...
  require$$0$5.app.on("second-instance", () => {
    if (mainWindow) {
