...
export class DatabaseMigrator {
  private database: MCPJamDatabase;
  ...
  extractLocalStorageData(): LocalStorageData | null {
    ...
    try {
      ...
      const appSettingsData: Record<string, any> = {};
   ...
   */
  async migrateData(data: LocalStorageData): Promise<MigrationResult> {
  ...
  async isMigrationNeeded(): Promise<boolean> {
    ...
    const hasLocalData = !!(
      localData.serverConfigs && Object.keys(localData.serverConfigs).length > 0 ||
  ...
  private convertToServerConfig(name: string, config: any): CreateServerConfigInput {
    const transportType = config.transportType || 'stdio';
    
    const serverConfig: CreateServerConfigInput = {
      name,
  ...
  private convertToRequestHistory(request: any, serverIdMap: Map<string, string>): CreateRequestHistoryInput | null {
  ...
  private convertToUserPreferences(prefs: any): UpdateUserPreferencesInput {
    return {
  ...
  private convertToProviderConfig(providerType: string, config: any): CreateProviderConfigInput {
    return {
...
export const createMigrator = (database: MCPJamDatabase) => new DatabaseMigrator(database);
