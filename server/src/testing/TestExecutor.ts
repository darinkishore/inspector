...
export class TestExecutor {
  ...
  private async initializeConnections(testCase: TestCase): Promise<Map<string, string>> {
    ...
    for (const serverConfig of testCase.serverConfigs) {
      try {
        ...
        if (serverConfig.type === 'stdio') {
          ...
          const connection = await this.mcpProxyService.createStreamableHTTPConnection(
            serverConfig,
            {}
          );
          sessionId = connection.sessionId;
  ...
  private async executeTestLogic(testCase: TestCase, connections: Map<string, string>): Promise<ToolCallRecord[]> {
  ...
  private async runTestWithConnections(testCase: TestCase, connections: Map<string, string>): Promise<ToolCallRecord[]> {
    const toolCalls: ToolCallRecord[] = [];
  ...
  private async cleanupConnections(connections: Map<string, string>): Promise<void> {
    for (const [serverId, sessionId] of connections.entries()) {
      try {
