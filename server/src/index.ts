...
app.post("/mcp", express.json({ limit: '10mb' }), async (req, res) => {
  ...
  if (!sessionId) {
    try {
      ...
      try {
        const { sessionId, webAppTransport } = await mcpProxyService.createStreamableHTTPConnection(
          serverConfig,
          headers
        );

        // Set the session ID header in the response so future requests can use it
